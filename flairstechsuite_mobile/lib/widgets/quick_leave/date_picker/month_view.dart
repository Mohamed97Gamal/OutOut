...
class MonthView extends StatefulWidget {
  ...
  final List<dynamic>? specialDates2;
...
class _MonthViewState extends State<MonthView> {
  ...
  void didUpdateWidget(MonthView oldWidget) {
    if (widget.height != oldWidget.height ||
        ...
        !DateRangePickerHelper.isDateCollectionEquals(
            widget.specialDates2, oldWidget.specialDates2) ||
...
class _MonthViewSingleSelectionRenderWidget
  ...
  _MonthViewSingleSelectionRenderWidget(
      ...
      : super(children: widgets as List<Widget>);
...
abstract class _IMonthView extends RenderBox
  ...
  List<dynamic>? _specialDates2;

  List<dynamic>? get specialDates2 => _specialDates2;

  set specialDates2(List<dynamic>? value) {
    if (DateRangePickerHelper.isDateCollectionEquals(_specialDates2, value)) {
      return;
    }

    _specialDates2 = value;
    if (childCount == 0) {
      markNeedsPaint();
    } else {
      markNeedsLayout();
...
void _drawMonthCellsAndSelection(PaintingContext context, Size size,
  ...
  for (int j = 0; j < viewCount; j++) {
    ...
    for (int i = 0; i < datesCount; i++) {
      ...
      final bool isSpecialDate2 =
          DateRangePickerHelper.isDateWithInVisibleDates(
              monthView.visibleDates, monthView.specialDates2, date);
      bool isDisabledDate = DateRangePickerHelper.isDateWithInVisibleDates(
          monthView.visibleDates, monthView.disableDatesCollection, date);
      if (!isDisabledDate &&
          monthView is _MonthViewExtendableRangeSelectionRenderObject &&
...
TextStyle? _updateTextStyle(
  ...
  if (isSpecialDate2) {
    final TextStyle? specialDate2TextStyle =
        monthView.cellStyle.specialDates2TextStyle as TextStyle? ??
            monthView.datePickerTheme!.specialDatesTextStyle;
    if (specialDate2TextStyle != null) {
      return specialDate2TextStyle;
    }
  }
...
Decoration? _updateDecoration(
  ...
  if (isSpecialDate2) {
    return monthView.cellStyle.specialDates2Decoration as Decoration?;
  }
  if (isBlackedDate) {
