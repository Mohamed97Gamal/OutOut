...
class VenuesDealsNowPage extends TabViewItem {
  final List<String> filterCategoriesIds;

  VenuesDealsNowPage({
    required this.filterCategoriesIds,
    Key? key,
  }) : super(title: "Deals Now", key: key);
      ...
      child: CustomPagedListView<VenueSummaryResponse>(
        ...
        initPageFuture: (pageKey) async {
          final request = new VenueFilterationRequest()
            ...
            ..timeFilter = VenueTimeFilter.deals_now
            ..typesIds = []
            ..categoriesIds = filterCategoriesIds;
          var venuesResult = await ApiRepo().venueClient.getVenues(request, pageKey, 7);
          return venuesResult.result.toPagedList();
        },
        itemBuilder: (context, item, index) {
          return VenueCardItem(venueSummaryResponse: item);
        },
      ),
    );
  }
