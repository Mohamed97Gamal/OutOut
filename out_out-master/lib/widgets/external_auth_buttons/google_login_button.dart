...
import 'package:out_out/utils/google_auth_utils.dart';
...
class GoogleLoginButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ElevatedButton.icon(
      onPressed: () async {
        var accessToken = await loginWithGoogle();
        if (accessToken == null) {
          return;
        }
        var loginResult = await showFutureProgressDialog<LoginResponseOperationResult>(
          ...
          initFuture: () async {
            final request = new ExternalAuthenticationRequest(
              externalLoginProvider: ExternalProvider.google,
      ...
      icon: UniversalImage(IconAssets.google),
